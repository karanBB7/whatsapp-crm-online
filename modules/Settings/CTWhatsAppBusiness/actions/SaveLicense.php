...
class Settings_CTWhatsAppBusiness_SaveLicense_Action extends Settings_Vtiger_Index_Action {
	public function process(Vtiger_Request $request) {
		global $adb,$site_URL, $current_user;
		$currenUserID = $current_user->id;
		$getLicenseQuery=$adb->pquery("SELECT * FROM vtiger_ctwhatsappbusiness_license_setting",array());
		...
		if($result_response['Apirespocetype'] == '1'){
			...
			if($numOfLicenseCount > 0){
				...
				$query=$adb->pquery("UPDATE vtiger_ctwhatsappbusiness_license_setting SET license_key=?, domain=?, expirydate=?, licensefield1=?, licensefield2=?, licensefield3=? WHERE id=?",array($License_Key, $site_URL, $expirydate, $orderId, $curversion, $latversion, $record));
			}else{
				$query=$adb->pquery("INSERT INTO vtiger_ctwhatsappbusiness_license_setting (license_key,status,domain,expirydate,licensefield1,licensefield2,licensefield3) values(?,?,?,?,?,?,?)",array($License_Key, 1, $site_URL, $expirydate, $orderId, $curversion, $latversion));
			}
