...
class Settings_LayoutEditor_Field_Action extends Settings_Vtiger_Index_Action {
    ...
    public function add(Vtiger_Request $request) {
        ...
        $relatefield = $request->get('torelatefield');
        $relatemodule = $request->get('torelatemodule');
        ...
        if($type == "RelationField"){
            $adb = PearDatabase::getInstance();
                $sql = "INSERT INTO vtiger_fieldmodulerel (`fieldid`, `module`, `relmodule`) VALUES (?,?,?)";
                $adb->pquery($sql, array($fieldid, $moduleName, $relatefield));
        }
        
        if($type == "RelationModule"){
            $adb = PearDatabase::getInstance();
            $moduleInstance = Vtiger_Module::getInstance($moduleName);
            $relateModule = Vtiger_Module::getInstance($relatemodule);
            $relationLabel  = $relatemodule;
            $moduleInstance->setRelatedList(
                $relateModule, $relationLabel, Array('ADD','SELECT')
            );
