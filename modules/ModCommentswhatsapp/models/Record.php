...
/**
 * ModCommentswhatsapp Record Model
 */
class ModCommentswhatsapp_Record_Model extends Vtiger_Record_Model {
			...
			if(is_array($fileDetails[0])){
				foreach($fileDetails as $index => $fileDetail){
					if(!empty($fileDetail)){
						...
						$attachmentDetails[$index]['url'] = 'index.php?module=ModCommentswhatsapp&action=DownloadFile&record='. $recordId .'&fileid='. $fileDetail['attachmentsid'];
					}
				}
			}
		}
	 ...
	 * @return ModCommentwhatsapp_Record_Model
	 */
	public static function getInstanceById($record, $module=null) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT vtiger_modcommentswhatsapp.*, vtiger_crmentity.smownerid,
					vtiger_crmentity.createdtime, vtiger_crmentity.modifiedtime FROM vtiger_modcommentswhatsapp
					INNER JOIN vtiger_crmentity ON vtiger_modcommentswhatsapp.modcommentsid = vtiger_crmentity.crmid
	...
	public function getParentCommentModel() {
		...
		if(!empty($recordId))
			return ModCommentswhatsapp_Record_Model::getInstanceById($recordId, 'ModCommentswhatsapp');
	 ...
	 * @return ModCommentswhatsapp_Record_Model if exits or null
	 */
	public static function getRecentComments($parentRecordId, $pagingModel = false){
		$recordInstances = array();
		$db = PearDatabase::getInstance();

		$listView = Vtiger_ListView_Model::getInstance('ModCommentswhatsapp');
	 ...
	 * @return ModCommentswhatsapp_Record_Model(s)
	 */
	public static function getAllParentComments($parentId) {
			$recordInstances = array();
			$db = PearDatabase::getInstance();
			$focus = CRMEntity::getInstance('ModCommentswhatsapp');
			$query = $focus->get_commentswhatsapp();
			if($query) {
	...
	public function getChildCommentsCount() {
		...
		$query = 'SELECT 1 FROM vtiger_modcommentswhatsapp WHERE parent_comments = ? AND related_to = ?';
