...
class Settings_CTWhatsAppBusiness_UpdateWhatsappModule_View extends Settings_Vtiger_Index_View {
	...
	function updateWhatsappModule(Vtiger_Request $request) {
		...
		$zipUrl = 'http://whatsappsocket.crmtiger.com/whatsappmodulezip/CTWhatsAppBusiness.zip';
		$downloadedFileContents = file_get_contents($zipUrl);
		mkdir($root_directory."/ZipFile/", 0755);
		$destinationPath = $root_directory."/ZipFile/CTWhatsAppBusiness.zip"; 
		chmod($root_directory."/ZipFile/CTWhatsAppBusiness.zip",0755);
		chmod($root_directory."/ZipFile/",0777);
		file_put_contents($destinationPath, $downloadedFileContents);
		
		$package = new Vtiger_Package();
		if(!getTabid('CTWhatsAppBusiness')){
			$package->import($root_directory."/ZipFile/CTWhatsAppBusiness.zip",true);
		}else{
			$package->update(Vtiger_Module::getInstance('CTWhatsAppBusiness'),$root_directory."/ZipFile/CTWhatsAppBusiness.zip");
		}
		$array = array('CTWhatsAppBusiness');
