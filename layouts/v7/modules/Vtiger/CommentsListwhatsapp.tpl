            ...
            {if is_array($PARENT_COMMENTS)}
                {foreach key=Index item=COMMENT from=$PARENT_COMMENTS}
                    ...
                    <li class="commentDetails">
                        {include file='Commentwhatsapp.tpl'|@vtemplate_path COMMENT=$COMMENT COMMENT_MODULE_MODEL=$COMMENTS_MODULE_MODEL}
                ...
                {/foreach}
            {else}
                {include file='Commentwhatsapp.tpl'|@vtemplate_path COMMENT=$PARENT_COMMENTS}
            {/if}
        </ul>
    {else}
        <div class="noCommentsMsgContainer" style='padding:20px;'>
            <p class="textAlignCenter">No Messages Sent</p>	
        </div>
    {/if}
    {/strip}
