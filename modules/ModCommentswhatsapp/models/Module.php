...
class ModCommentswhatsapp_Module_Model extends Vtiger_Module_Model{
	...
	static function storeRollupSettingsForUser($currentUserModel, $request) {
		...
		if (!$rollupid) {
			...
			$query = "INSERT INTO vtiger_rollupcommentswhatsapp_settings(userid, tabid, rollup_status)"
					. " VALUES(" . generateQuestionMarks($params) . ")";
			$db->pquery($query, $params);
			return ModCommentswhatsapp_Module_Model::getRollupSettingsForUser($currentUserModel, $request->get('parent'));
		} else {
			$params = array($rollupStatus, $userid, $tabid);
			$query = "UPDATE vtiger_rollupcommentswhatsapp_settings set rollup_status=?"
	...
	static function getRollupSettingsForUser($currentUserModel, $modulename) {
		...
		$query = 'SELECT rollupid, rollup_status FROM vtiger_rollupcommentswhatsapp_settings WHERE userid=? AND tabid=?';
