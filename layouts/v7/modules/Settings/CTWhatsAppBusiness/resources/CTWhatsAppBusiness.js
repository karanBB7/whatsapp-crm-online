...
jQuery.Class("Settings_CTWhatsAppBusiness_CTWhatsAppBusiness_Js",{
        ...
        var auth_token = jQuery('input[name="auth_token"]').val();
            if(License_Key == ''){
                ...
                AppConnector.request(params).then(function(data) {
                        ...
                        //var redirectUrl = 'index.php?parent=Settings&module=CTWhatsAppBusiness&view=ConfigurationEdit&userid='+currenUserID;
    ...
    registerEventsForUserWhatsAppLogout : function() {
        jQuery('.selectUserLogout').on('click', function (e) {
            ...
            var message1 = app.vtranslate("Are you sure to Logout?");
                app.helper.showConfirmationBox({'message' : message1}).then(function(data) {
                var dataUrl = 'index.php?module=CTWhatsAppBusiness&parent=Settings&view=WhatsappChatConfiguration&mode=deleteWhatsAppUsers&selectUserid=yes&userid='+whatsappuserid;
                window.location.href = dataUrl;
            });
        });
    },
            ...
            AppConnector.request(params).then(
                ...
                app.showModalWindow(data, function(data){
                    ...
                    jQuery('#saveMultipleWhatsapp').on('click', function (e) {
                        ...
                        var whatsapp_appid = jQuery('#whatsapp_appid').val();
                        if(whatsapp_appid.trim() == ''){
                            var message1 = app.vtranslate("Please enter App Id");
                            app.helper.showErrorNotification({'title': 'Error', 'message': message1});
                            return false;
                        }
                        var whatsapp_accountid = jQuery('#whatsapp_accountid').val();
                        if(whatsapp_accountid.trim() == ''){
                            var message2 = app.vtranslate("Please enter Whatsapp Business Account Id");
                            app.helper.showErrorNotification({'title': 'Error', 'message': message2});
                            return false;
                        }
                        var access_token = jQuery('#access_token').val();
                        if(access_token.trim() == ''){
                            var message3 = app.vtranslate("Please enter User Access Token");
                            app.helper.showErrorNotification({'title': 'Error', 'message': message3});
                            return false;
                        }
                        var whatsappno = jQuery('#whatsappno').val();
                        if(whatsappno.trim() == ''){
                            var message4 = app.vtranslate("Please enter WhatsApp Business No");
                            app.helper.showErrorNotification({'title': 'Error', 'message': message4});
                            return false;
                        }
                        var whatsapp_businessnumber = jQuery('#whatsapp_businessnumber').val();
                        if(whatsapp_businessnumber.trim() == ''){
                            var message4 = app.vtranslate("Please enter Phone Number ID");
                            app.helper.showErrorNotification({'title': 'Error', 'message': message4});
                            return false;
                        }
                        if(scanWhatsappUsersGroups == ''){
                            var message4 = app.vtranslate("Please select any Scans WhatsApp Business");
                            app.helper.showErrorNotification({'title': 'Error', 'message': message4});
                            return false;
                        }
                        var params = {
                            ...
                            'whatsapp_appid' : whatsapp_appid,
                            'whatsapp_accountid' : whatsapp_accountid,
                            'access_token' : access_token,
                            'whatsappno' : whatsappno,
                            'whatsapp_businessnumber' : whatsapp_businessnumber
                        }
                        app.helper.showProgress();
                        AppConnector.request(params).then(
                            function(data) {
...
jQuery(document).ready(function(){
    var thisInstance = new Settings_CTWhatsAppBusiness_CTWhatsAppBusiness_Js();
