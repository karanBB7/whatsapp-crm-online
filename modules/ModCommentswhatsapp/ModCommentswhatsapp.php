 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once dirname(__FILE__) . '/ModCommentswhatsappCore.php';
include_once dirname(__FILE__) . '/models/Comments.php';

require_once 'include/utils/VtlibUtils.php';

class ModCommentswhatsapp extends ModCommentswhatsappCore {
	...
	static function transferRecords($currentParentId, $targetParentId) {
		global $adb;
		$adb->pquery("UPDATE vtiger_modcommentswhatsapp SET related_to=? WHERE related_to=?", array($targetParentId, $currentParentId));
	}
	...
	static function getWidget($name) {
		if ($name == 'DetailViewBlockCommentWidget' &&
				isPermitted('ModCommentswhatsapp', 'DetailView') == 'yes') {
			require_once dirname(__FILE__) . '/widgets/DetailViewBlockComment.php';
			return (new ModCommentswhatsapp_DetailViewBlockCommentWidget());
		}
		return false;
	}
	...
	static function addWidgetTo($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		$modCommentswhatsappModule = Vtiger_Module::getInstance('ModCommentswhatsapp');
		
		$commentWidgetModules = array();
		foreach($moduleNames as $moduleName) {
			$module = Vtiger_Module::getInstance($moduleName);
			if($module) {
				$module->addLink($widgetType, $widgetName, "block://ModCommentswhatsapp:modules/ModCommentswhatsapp/ModCommentswhatsapp.php");
				$module->setRelatedList($modCommentswhatsappModule, 'ModCommentswhatsapp', array(''), 'get_comments');
				$commentWidgetModules[] = $moduleName;
			}
		}
		if (php7_count($commentWidgetModules) > 0) {
			$modCommentswhatsappModule->addLink('HEADERSCRIPT', 'ModCommentswhatsappCommonHeaderScript', 'modules/ModCommentswhatsapp/ModCommentswhatsappCommon.js');
			$modCommentswhatsappRelatedToField = Vtiger_Field::getInstance('related_to', $modCommentswhatsappModule);
			$modCommentswhatsappRelatedToField->setRelatedModules($commentWidgetModules);
		}
	}
	...
	static function removeWidgetFrom($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		foreach($moduleNames as $moduleName) {
			$module = Vtiger_Module::getInstance($moduleName);
			if($module) {
				$module->deleteLink($widgetType, $widgetName, "block://ModCommentswhatsapp:modules/ModCommentswhatsapp/ModCommentswhatsapp.php");
				$module->unsetRelatedList($modCommentswhatsappModule, 'ModCommentswhatsapp', 'get_comments');
				$commentWidgetModules[] = $moduleName;
			}
		}
		if (php7_count($commentWidgetModules) > 0) {
			$modCommentswhatsappRelatedToField = Vtiger_Field::getInstance('related_to', $modCommentswhatsappModule);
			$modCommentswhatsappRelatedToField->unsetRelatedModules($commentWidgetModules);
		}
	}
	...
	function getAsCommentModel() {
		return new ModCommentswhatsapp_CommentsModel($this->column_fields);
	}
	...
	public static function copyCommentsToRelatedRecord($currentParentId, $targetParentId) {
		...
		$result = $db->pquery("SELECT *FROM vtiger_modcommentswhatsapp WHERE related_to=?", array($currentParentId));
...
// print_r($crmentityParams);
			...
			$db->pquery('INSERT INTO vtiger_modcommentswhatsapp values('. generateQuestionMarks($modcommentswhatsappParams) .')', $modcommentswhatsappParams);
