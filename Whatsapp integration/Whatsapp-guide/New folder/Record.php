...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	function getRollupCommentsForModulewhatsapp($startIndex = 0, $pageLimit = 10) {
		...
		if ($relatedModuleRecordIds) {
			...
			$query .= " AND vtiger_whatsapp.related_to IN (" . generateQuestionMarks($relatedModuleRecordIds)
					. ") AND vtiger_whatsapp.parent_comments=0 ORDER BY vtiger_crmentity.createdtime DESC LIMIT "
					. " $startIndex,$pageLimit";
			...
			if ($db->num_fields($result)) {
				for ($i = 0; $i < $db->num_rows($result); $i++) {
					$rowdata = $db->query_result_rowdata($result, $i);
					$recordInstance = new whatsapp_Record_Model();
					$rowdata['module'] = getSalesEntityType($rowdata['related_to']);
					$recordInstance->setData($rowdata);
					$rollupComments[] = $recordInstance;
				}
