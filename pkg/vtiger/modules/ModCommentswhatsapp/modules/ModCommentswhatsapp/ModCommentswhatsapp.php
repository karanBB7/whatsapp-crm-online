...
class ModCommentswhatsapp extends ModCommentswhatsappCore {
	...
	static function addWidgetTo($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		$modCommentsModule = Vtiger_Module::getInstance('ModCommentswhatsapp');
		...
		foreach($moduleNames as $moduleName) {
			...
			if($module) {
				...
				$module->setRelatedList($modCommentsModule, 'ModCommentswhatsapp', array(''), 'get_comments');
				$commentWidgetModules[] = $moduleName;
			}
		}
		if (php7_count($commentWidgetModules) > 0) {
			$modCommentsModule->addLink('HEADERSCRIPT', 'ModCommentswhatsappCommonHeaderScript', 'modules/ModCommentswhatsapp/ModCommentswhatsappCommon.js');
	...
	static function removeWidgetFrom($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		foreach($moduleNames as $moduleName) {
			...
			if($module) {
				...
				$module->unsetRelatedList($modCommentsModule, 'ModCommentswhatsapp', 'get_comments');
				$commentWidgetModules[] = $moduleName;
			}
		}
		if (php7_count($commentWidgetModules) > 0) {
	...
	public static function copyCommentsToRelatedRecord($currentParentId, $targetParentId) {
		...
		for($i=0;$i<$count;$i++) {
			...
			$db->pquery('INSERT INTO vtiger_modcommentswhatsapp values('. generateQuestionMarks($modcommentsParams) .')', $modcommentsParams);
