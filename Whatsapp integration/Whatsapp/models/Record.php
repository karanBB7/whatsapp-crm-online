 ...
 */
class Whatsapp_Record_Model extends Vtiger_Record_Model {
			...
			if(is_array($fileDetails[0])){
				foreach($fileDetails as $index => $fileDetail){
					if(!empty($fileDetail)){
						...
						$attachmentDetails[$index]['url'] = 'index.php?module=Whatsapp&action=DownloadFile&record='. $recordId .'&fileid='. $fileDetail['attachmentsid'];
					}
				}
			}
		}
	 ...
	 * @return Whatsapp_Record_Model
	 */
	public static function getInstanceById($record, $module=null) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT vtiger_whatsapp.*, vtiger_crmentity.smownerid,
					vtiger_crmentity.createdtime, vtiger_crmentity.modifiedtime FROM vtiger_whatsapp
					INNER JOIN vtiger_crmentity ON vtiger_whatsapp.modcommentsid = vtiger_crmentity.crmid
	...
	public function getParentCommentModel() {
		...
		if(!empty($recordId))
			return Whatsapp_Record_Model::getInstanceById($recordId, 'Whatsapp');
	 ...
	 * @return Whatsapp_Record_Model if exits or null
	 */
	public static function getRecentComments($parentRecordId, $pagingModel = false){
		$recordInstances = array();
		$db = PearDatabase::getInstance();

		$listView = Vtiger_ListView_Model::getInstance('Whatsapp');
	 ...
	 * @return Whatsapp_Record_Model(s)
	 */
	public static function getAllParentComments($parentId) {
			$recordInstances = array();
			$db = PearDatabase::getInstance();
			$focus = CRMEntity::getInstance('Whatsapp');
	...
	public function getChildCommentsCount() {
		...
		$query = 'SELECT 1 FROM vtiger_whatsapp WHERE parent_comments = ? AND related_to = ?';
	...
	static function getAllComments($parentId) {
		$db = PearDatabase::getInstance();
		$focus = CRMEntity::getInstance('Whatsapp');
		$query = $focus->get_commentsswhatsapp();
		if($query) {
