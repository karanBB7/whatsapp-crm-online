...
<div id="sendWhatsappSmsContainer" class='modal-xs modal-dialog' style="background: #fbfbfb;">
    ...
    <div class = "modal-content">
		<table class="table table-borderless">
			...
			<tr class="hide">
				<td colspan="2">
					<textarea name="newTextMessage" id="newTextMessage" style="width: 100%;height: 170px;" placeholder="{vtranslate('TYPE_MESSAGE', $MODULE)}"></textarea>
				</td>
			</tr>
			<tr class="hide">
				<td colspan="2">
			...
			<tr>
				<td colspan="2" style="width: 19%;">
					<b>{vtranslate('MSGTEMPLATES', $MODULE)} : </b><select class="select2" name="wptemplatesid" id="wptemplatesid" style="width: 60%;">
						<option value="">{vtranslate('LBL_SELECTFROMNUMBER',$MODULE)}</option>
						{foreach item=wpTemplates_VALUE key=wpTemplates_KEY from=$wpTemplates}
							<option value="{$wpTemplates_KEY}">{$wpTemplates_VALUE}</option>
						{/foreach}
					</select>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<input type="hidden" name="whatsappTemplateid" id="whatsappTemplateid" value="">
