...
class Vtiger_Menu_Model extends Vtiger_Module_Model {
	...
	public static function getAll($presence = array(), $restrictedModulesList = array(),$sequenced = false) {
		...
		$restrictedModulesList = array('Emails', 'ProjectMilestone', 'ProjectTask', 'ModComments','ModCommentswhatsapp', 'Integration', 'PBXManager', 'Dashboard', 'Home');
	...
	public static function getAllForQuickCreate() {
		$userPrivModel = Users_Privileges_Model::getCurrentUserPrivilegesModel();
		$restrictedModulesList = array('Emails', 'ModComments','ModCommentswhatsapp', 'Integration', 'PBXManager', 'Dashboard', 'Home');
		$allModules = parent::getAll(array('0', '2'));
		$menuModels = array();
