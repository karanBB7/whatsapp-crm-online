...
class Settings_LayoutEditor_Module_Model extends Vtiger_Module_Model {
	...
	public function getAddSupportedFieldTypes() {
		return array(
			...
			'URL','Checkbox','TextArea','MultiSelectCombo','Skype','Time','RelationField','RelationModule'
		);
	}
	...
	public function addField($fieldType, $blockId, $params) {
		...
		$keylabel = $params['fieldLabel'] = trim($params['KeyLabel']);
		...
		if (isset($focus->customFieldTable)) {
			...
			$tableName = 'vtiger_' . strtolower($moduleName, 0). 'cf';
		}
