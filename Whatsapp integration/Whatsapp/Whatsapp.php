 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once dirname(__FILE__) . '/WhatsappCore.php';
include_once dirname(__FILE__) . '/models/Comments.php';

require_once 'include/utils/VtlibUtils.php';

class Whatsapp extends WhatsappCore {
	...
	static function transferRecords($currentParentId, $targetParentId) {
		global $adb;
		$adb->pquery("UPDATE vtiger_whatsapp SET related_to=? WHERE related_to=?", array($targetParentId, $currentParentId));
	}
	...
	static function getWidget($name) {
		if ($name == 'DetailViewBlockCommentWidget' &&
				isPermitted('Whatsapp', 'DetailView') == 'yes') {
			require_once dirname(__FILE__) . '/widgets/DetailViewBlockComment.php';
			return (new Whatsapp_DetailViewBlockCommentWidget());
		}
		return false;
	}
	...
	static function addWidgetTo($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		foreach($moduleNames as $moduleName) {
			...
			if($module) {
				$module->addLink($widgetType, $widgetName, "block://Whatsapp:modules/Whatsapp/Whatsapp.php");
				$module->setRelatedList($whatsappModule, 'Whatsapp', array(''), 'get_comments');
				$commentWidgetModules[] = $moduleName;
			}
		}
		if (php7_count($commentWidgetModules) > 0) {
			$whatsappModule->addLink('HEADERSCRIPT', 'WhatsappCommonHeaderScript', 'modules/Whatsapp/WhatsappCommon.js');
			$whatsappRelatedToField = Vtiger_Field::getInstance('related_to', $whatsappModule);
			$whatsappRelatedToField->setRelatedModules($commentWidgetModules);
		}
	}
	...
	static function removeWidgetFrom($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		foreach($moduleNames as $moduleName) {
			$module = Vtiger_Module::getInstance($moduleName);
			if($module) {
				$module->deleteLink($widgetType, $widgetName, "block://Whatsapp:modules/Whatsapp/Whatsapp.php");
				$module->unsetRelatedList($whatsappModule, 'Whatsapp', 'get_comments');
				$commentWidgetModules[] = $moduleName;
			}
		}
		if (php7_count($commentWidgetModules) > 0) {
			$whatsappRelatedToField = Vtiger_Field::getInstance('related_to', $whatsappModule);
			$whatsappRelatedToField->unsetRelatedModules($commentWidgetModules);
		}
	}
	...
	function getAsCommentModel() {
		return new Whatsapp_CommentsModel($this->column_fields);
	}
	...
	public static function copyCommentsToRelatedRecord($currentParentId, $targetParentId) {
		...
		$result = $db->pquery("SELECT *FROM vtiger_whatsapp WHERE related_to=?", array($currentParentId));
...
// print_r($crmentityParams);
			...
			$whatsappParams = array($commentCrmId, $commentContent, 
			...
			$db->pquery('INSERT INTO vtiger_whatsapp values('. generateQuestionMarks($whatsappParams) .')', $whatsappParams);
