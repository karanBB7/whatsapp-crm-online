...
class CRMEntity {
	...
	function initialize() {
		...
		$userSpecificTableIgnoredModules = array('SMSNotifier', 'PBXManager', 'ModComments', 'ModCommentswhatsapp');
...
// for whatsapp
	...
	function save($module_name, $fileid = '') {
		...
		if($em) {
			...
			if($module_name == "ModCommentswhatsapp"){
...
//Added by Richie
	...
	function get_commentswhatsapp($relatedRecordId = false) {
		$current_user = vglobal('current_user');
		$moduleName = $this->moduleName;
		if($moduleName != 'ModCommentswhatsapp') {
			return false;
		}
		$queryGenerator = new EnhancedQueryGenerator($moduleName, $current_user);
		if(is_object($this->column_fields)) {
