...
class ModCommentswhatsapp_InRelation_View extends Vtiger_RelatedList_View {
	...
	function process(Vtiger_Request $request) {
		...
		$modCommentswhatsappModel = Vtiger_Module_Model::getInstance('ModCommentswhatsapp');
		...
		if ($rollupStatus) {
			...
			$parentCommentModels = ModCommentswhatsapp_Record_Model::getAllParentComments($parentRecordId);
		}

		$currentCommentModel = null;
		if (!empty($commentRecordId)) {
			$currentCommentModel = ModCommentswhatsapp_Record_Model::getInstanceById($commentRecordId);
		}

		// To get field model of filename
		$fileNameFieldModel = Vtiger_Field::getInstance("filename", $modCommentswhatsappModel);
		$fileFieldModel = Vtiger_Field_Model::getInstanceFromFieldObject($fileNameFieldModel);
		$viewer = $this->getViewer($request);
		$viewer->assign('CURRENTUSER', $currentUserModel);
		$viewer->assign('COMMENTS_MODULE_MODEL', $modCommentswhatsappModel);
